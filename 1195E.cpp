...
template <typename T>
vector<int> sliding_window(const vector<int> &v, int k, const function<bool(T, T)> &f) {
  int n = (int) v.size();
  assert(0 < k && k <= n);
  vector<int> res;
  deque<int> dq;
  ...
  // return res;
  // returns the index corresponding to the first value according to f
  // breaking ties with the lower-indexed value
