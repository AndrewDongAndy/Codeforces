...
Not a very tricky DP problem.

First of all, clearly we should compress coordinates.

This solution does "two layers" of DP: one on the segments and
one on the x-coordinates within each segment. To make things easier,
add an interval covering everything so we only have to worry about
complete nesting inside one interval.

A more standard approach might be to solve the problem for all
intervals [L, R], but I couldn't see it at first. The editorial
