...
This solution takes so long to run locally on a max case but takes < 2s on CF?!

Also, there is a solution with just two BITs:
- one to maintain the counts of each a[i], sweeping in increasing i
- one to maintain sums with O(n log n) range updates; this BIT is used as
  a difference array

But at least I solved it. lol
*/
