...
void bfs(int x) {
  ...
  while (!q.empty()) {
    ...
    for (int id : g[v]) {
      if (used[id] == x) {
        continue; // take each edge at most once
      }
      used[id] = x;
...
int main() {
  ...
  // every edge in any cycle in this graph must be between 2 nodes such that
  // one of the nodes corresponds to a prime <= 1000
